...
├── slide4_ensemble.py            # アンサンブル数 m=2,3,8,20 比較
├── slide_m2_heatmap.py           # m=2 ヒートマップ（最適固定値の確認）
├── slide_m2_vb.py                # m=2: v_b 感度分析（動的インフレーション）
...
# アンサンブル数 m=2,3,8,20 の比較
python3 slide4_ensemble.py

# m=2: ヒートマップ（最適固定値の確認）
python3 slide_m2_heatmap.py

# m=2: v_b 感度分析
python3 slide_m2_vb.py
```
...
### アンサンブル数 m による比較（v_b=0.04²）

| m | σ | 固定 Δ | 動的の平均Δ | 固定 RMSE | 動的 RMSE |
|---|---|---|---|---|---|
| **2** | **1.0** | **1.80** | **1.803** | **0.555** | 0.592 |
| 3 | 2.0 | 1.30 | 1.309 | 0.395 | **0.372** |
| 8 | 6.0 | 1.10 | 1.103 | 0.231 | **0.222** |
| 20 | 10.0 | 1.02 | 1.016 | 0.177 | 0.179 |

m=2 では動的がデフォルト v_b=0.04² で固定より悪化（+6.7%）。m=3,8 では動的が優位。m=20 はほぼ同等。

### m=2：v_b 感度分析

m=2 ではデフォルト v_b（=0.04²）で Δ 推定が不安定になるため、v_b を小さくすると改善する。
...
| 動的 v_b=0.01² | 0.560 | 1.800 |
| 動的 v_b=0.005² | **0.542** | 1.800 |

v_b を小さくすることでΔ推定が安定し、固定より低い RMSE を達成できる。
...
**Δ のクリップ**：数値安定性のため Δ ∈ [1.0, 2.0] に制限している（[adaptive_inflation.py:48](function/adaptive_inflation.py)）。

**局所性**：Δ推定は各格子点の局所観測のみを使い、格子点ごとに独立した Δ を保持する。

**m=2 での v_b 感度**：m が極端に小さい（=2）場合、デフォルト v_b=0.04² では Δ 推定が更新されすぎて不安定になる。v_b を 0.005²〜0.02² に下げると固定最適値を下回るRMSEを達成できる（[slide_m2_vb.py](slide_m2_vb.py) 参照）。
